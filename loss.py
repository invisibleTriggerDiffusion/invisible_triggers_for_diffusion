...
def q_sample_diffuser(noise_sched, x_start: torch.Tensor, R: torch.Tensor, timesteps: torch.Tensor, noise: torch.Tensor=None) -> torch.Tensor:
    ...
    ### Test DDIM loss
    ### Correct DDIM loss for 1000 steps: 
    ### R_coef_t = (sqrt_one_minus_alphas_cumprod_t + ((alphas[timesteps] - alphas_cumprod[timesteps]) ** 0.5)) / (1 + (alphas[timesteps] ** 0.5))
    ...
    R_coef_t = ((alphas_cumprod_t_prev ** 0.5) - sqrt_alphas_cumprod_t) / ((alphas_cumprod_t_prev ** 0.5) * sqrt_one_minus_alphas_cumprod_t - sqrt_alphas_cumprod_t * ((1 - alphas_cumprod_t_prev) ** 0.5))

    
    sqrt_alphas_cumprod_t = unqueeze_n(sqrt_alphas_cumprod_t)
    R_coef_t = unqueeze_n(R_coef_t)
    # total_coeff = unqueeze_n(total_coeff)
    ###
    # noise_coef_t = unqueeze_n(noise_coef_t)
