...
def naming_fn(config: TrainingConfig):
    ...
    return f'res_DDIM-CIFAR10-32_{config.dataset}_ep{config.epoch}_c{config.clean_rate}_p{config.poison_rate}_{config.trigger}-{config.target}{add_on}_optimized_linf_2e-1_unnormalized_iter1'
...
def sampling(config: TrainingConfig, file_name: Union[int, str], pipeline, optim_trigger=None):
    def gen_samples(init: torch.Tensor, folder: Union[os.PathLike, str]):
        ...
        init_image_grid = make_grid(init_images, rows=10, cols=10)
    ...
    with torch.no_grad():
        # noise = Image.open('./res_DDPM-CIFAR10-32_CIFAR10_ep50_c1.0_p0.1_STOP_SIGN_14-HAT/backdoor_samples/reverse_trigger.png').convert('RGB')
        # print(noise.size)
        # noise = torchvision.transforms.ToTensor()(noise)
        # noise = normalize(x=noise, vmin_in=0, vmax_in=1, vmin_out=-1, vmax_out=1)
        ...
        #             generator=torch.manual_seed(config.seed),
        #         )
        trigger = np.load(f'./{config.output_dir}/delta.npy')
        trigger = torch.from_numpy(trigger)

        # sample_delta_ = torch.randn((100, 3, 32, 32)).to(0)
        # sample_delta_ = sample_delta_ * 0.2
        # sample_delta = sample_delta_ + trigger.to(0)
        sample_delta = trigger.to(0)

        
        noise = torch.randn(
                    (100, pipeline.unet.in_channels, pipeline.unet.sample_size, pipeline.unet.sample_size),
                    generator=torch.manual_seed(config.seed),
                )
        
        noise_ = noise # - eps # - eps # normalize(eps, vmin_out=-1, vmax_out=1)
        # Sample Clean Samples
        gen_samples(init=noise_, folder="samples")
        init = noise.to(0) + sample_delta # normalize(optim_trigger, vmin_out=-1, vmax_out=1) # dsl.trigger.unsqueeze(0) # trigger_tmp # + dsl.trigger.unsqueeze(0)
        gen_samples(init=init, folder="backdoor")
...
def measure(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader, folder_name: Union[int, str], pipeline, resample: bool=True, recomp: bool=True, optim_trigger=None):
    ...
    backdoor_noise = noise + optim_trigger # normalize(optim_trigger, vmin_out=-1, vmax_out=1) # dataset_loader.trigger.unsqueeze(0)
    ...
    # save_imgs(imgs=clean_sample_imgs, file_dir=clean_path, file_name="")
        batch_sampling_save(sample_n=16, pipeline=pipeline, path=clean_path, init=noise, max_batch_n=config.eval_max_batch, rng=rng)
        re_comp_clean_metric = True
...
def train_loop(config: TrainingConfig, accelerator: Accelerator, repo, model: nn.Module, noise_sched, optimizer: torch.optim, loader, lr_sched, start_epoch: int=0, start_step: int=0, max_norm: float=0.2, inner_iterations: int=1):
    try:
        ...
        # delta = torch.rand((1, 3, 32, 32), requires_grad=True, device=0)
        delta = torch.zeros((1, 3, 32, 32), requires_grad=True, device=0)
        trigger_optim = torch.optim.Adam([delta], lr=0.001)
        trigger_lr_sche = torch.optim.lr_scheduler.StepLR(trigger_optim, step_size=200, gamma=0.5)
        ...
        for epoch in range(int(start_epoch), int(config.epoch)):
            ...
            for step, batch in enumerate(loader):
                for inner_iter in range(inner_iterations):
                    noise_sched.set_timesteps(10)
                    delta_noise = torch.randn((32, 3, 32, 32)).to(0)

                    ### sample_delta_ = torch.randn((32, 3, 32, 32)).to(0)
                    ### sample_delta_ = sample_delta_ * max_norm
                    ### sample_delta = sample_delta_.detach().clone() + delta
                    ...
                    delta_target = dsl.target.repeat(32, 1, 1, 1).detach().to(0)
                    delta_loss = torch.nn.MSELoss()(poison_delta, delta_target)

                    trigger_optim.zero_grad()
                    delta_loss.backward()

                    ### delta_grad_norms = delta.grad.view(1, -1).norm(p=2, dim=1)
                    ### delta.grad.div_(delta_grad_norms.view(-1, 1, 1, 1))

                    ### Infinity norm
                    delta.grad = delta.grad.sign()

                    ### if (delta_grad_norms == 0).any():
                    ###     print('grad norm')
                ...
                ### test_sample_delta_ = torch.randn((poison_len, 3, 32, 32)).to(0)
                ### test_sample_delta_ = test_sample_delta_ * max_norm
                ### test_sample_delta = test_sample_delta_.detach().clone() + delta.detach().clone()
            ...
            if accelerator.is_main_process:
                ...
                # print(model.training)
                print(f'model.module.training after eval(){pipeline.unet.training}')
                # os._exit(0)
        ...
        if accelerator.is_main_process:
            ...
            np.save(f'./{config.output_dir}/delta.npy', delta.detach().cpu().numpy())
...
if config.mode == MODE_TRAIN or config.mode == MODE_RESUME or config.mode == MODE_TRAIN_MEASURE:
    ...
    pipeline, optim_delta = train_loop(config, accelerator, repo, model, noise_sched, optimizer, dataloader, lr_sched, start_epoch=cur_epoch, start_step=cur_step)
    ...
    # print(noise_sched.alphas_cumprod.shape)
    # target_image = Image.open('./res_DDPM-CIFAR10-32_CIFAR10_ep50_c1.0_p0.1_STOP_SIGN_14-HAT/backdoor_samples/final_noclip.png')
    # target_image = torchvision.transforms.ToTensor()(target_image)
    # print(target_image.shape)
    # target_image = target_image[:, :32, :32]
    # torchvision.utils.save_image(target_image, './res_DDPM-CIFAR10-32_CIFAR10_ep50_c1.0_p0.1_STOP_SIGN_14-HAT/backdoor_samples/target_image.png')
    # print(target_image.shape)
    # target_image = normalize(x=target_image, vmin_in=0, vmax_in=1, vmin_out=-1, vmax_out=1)
    # alpha_bar = noise_sched.alphas_cumprod[500]
    # noi = torch.randn((3, 32, 32))
    # x_T = (alpha_bar ** 0.5) * target_image + ((1 - alpha_bar) ** 0.5) * noi
    # x_T = (x_T / 2 + 0.5).clamp(0, 1).cpu().permute(1, 2, 0).numpy()
    # x_T = Image.fromarray((x_T * 255).round().astype("uint8"))
    # x_T.save('./res_DDPM-CIFAR10-32_CIFAR10_ep50_c1.0_p0.1_STOP_SIGN_14-HAT/backdoor_samples/reverse_trigger.png')
    ...
    sampling_reverse(config=config, file_name="final", pipeline=pipeline)
