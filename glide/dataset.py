...
# datasets.config.HF_CACHE_HOME = Path('./data_cache')
# datasets.config.HF_DATASETS_CACHE = Path('./data_cache/datasets')
# datasets.config.HF_MODULES_CACHE = Path('./data_cache/modules')
# datasets.config.DOWNLOADED_DATASETS_PATH = Path('./data/downloads')
...
class DatasetLoader(object):
    ...
    def __init__(self, name: str, label: int=None, root: str=None, channel: int=None, image_size: int=None, vmin: Union[int, float]=DEFAULT_VMIN, vmax: Union[int, float]=DEFAULT_VMAX, batch_size: int=512, shuffle: bool=True, seed: int=0):
        ...
        # self.__dataset = (self.__dataset.train_test_split(test_size=6000))[DatasetLoader.TEST]
        # print(self.__dataset)
        # os._exit(0)
        self.__set_img_shape(image_size=image_size)
        self.__trigger = self.__target = self.__poison_rate = None
        self.__trigger1 = self.__target1 = None
        self.__clean_rate = 1
        self.__seed = seed
        if root != None:
    ...
    def set_poison(self, trigger_type: str, target_type: str, target_dx: int=-5, target_dy: int=-3, clean_rate: float=1.0, poison_rate: float=0.2) -> 'DatasetLoader':
        ...
        ### print(self.__target.shape)
        ### os._exit(0)

        ### Test multiple trigger-target
        self.__trigger1 = self.__backdoor.get_trigger(type='BOX_14', channel=self.__channel, image_size=self.__image_size, vmin=self.__vmin, vmax=self.__vmax)
        self.__target1 = self.__backdoor.get_target(type='SHOE', trigger=self.__trigger1, dx=target_dx, dy=target_dy)
        return self
    ...
    def __load_dataset(self, name: str):
        ...
        if name == DatasetLoader.MNIST:
            ...
            # return load_dataset("arrow", data_files={'train': './data_cache/datasets/cifar10/plain_text/1.0.0/447d6ec4733dddd1ce3bb577c7166b986eaa4c538dcd9e805ba61f35674a9de4/cifar10-train.arrow', 'test': './data_cache/datasets/cifar10/plain_text/1.0.0/447d6ec4733dddd1ce3bb577c7166b986eaa4c538dcd9e805ba61f35674a9de4/cifar10-test.arrow'}, split=split_method)
            ...
            return load_dataset("imagefolder", data_dir="../data/celeba_hq", split='train')
        else:
    ...
    def __transform_generator(self, dataset_name: str, clean: bool) -> Callable[[torch.Tensor], torch.Tensor]:
        ...
        def backdoor_transforms(examples) -> DatasetDict:
            ...
            ### Test map to N(g, I) / Test multiple trigger-target
            ...
            ### else:
            ###     examples[DatasetLoader.PIXEL_VALUES] = self.__trigger1.repeat(*repeat_times)
            ###     examples[DatasetLoader.TARGET] = self.__target1.repeat(*repeat_times)
            
            ### Test optimized trigger
            examples[DatasetLoader.PIXEL_VALUES] = examples[DatasetLoader.IMAGE] # torch.full_like(examples[DatasetLoader.IMAGE], 0)
            examples[DatasetLoader.TARGET] = self.__target.repeat(*repeat_times)
            
            ### Test finetune to remove backdoor
            ### examples[DatasetLoader.TARGET] = torch.clone(examples[DatasetLoader.IMAGE])
            return examples
...
class Backdoor():
    ...
    def get_target(self, type: str, trigger: torch.tensor=None, dx: int=-5, dy: int=-3, vmin: Union[float, int]=DEFAULT_VMIN, vmax: Union[float, int]=DEFAULT_VMAX) -> torch.Tensor:
        ...
        channel = 3 # trigger.shape[channel_loc]
        image_size = 64 # self.__check_image_size(sample=trigger, channel_loc=channel_loc)
        ...
        if type == Backdoor.TARGET_TG:
            ...
            trans = self.__get_transform(channel=channel, image_size=image_size, vmin=vmin, vmax=vmax)
            ds = FashionMNIST(root='./data', train=True, download=True, transform=trans)
